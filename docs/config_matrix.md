...
| Env Var / Surface | Default | Allowed | Meaning | Safe for prod? |
|---|---|---|---|---|
...
| EDGE_AUTH_MODE | unset | `dev_allow_anon`  `prod_enforce` | Explicit auth mode when enabled | Yes (required if enabled) |
...
| UI role selector | `auditor` | role enum | Client-side gating only; no auth enforcement | No (UI only) |
